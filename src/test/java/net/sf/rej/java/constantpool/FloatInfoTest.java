...
public class FloatInfoTest extends TestCase {
	...
	public final void testFloatInfoFloatConstantPool() {
		...
		Assert.assertEquals("0.0f", 0.0f, fi.getFloatValue(), 0f);
		fi = new FloatInfo(1.1f, new ConstantPool());
		Assert.assertEquals("1.1f", 1.1f, fi.getFloatValue(), 0f);
		fi = new FloatInfo(-1.1f, new ConstantPool());
		Assert.assertEquals("-1.1f", -1.1f, fi.getFloatValue(), 0f);
	}
	...
	public final void testGetFloatValue() {
		...
		Assert.assertEquals("1.0f", 1.0f, fi.getFloatValue(), 0f);
		fi = new FloatInfo(0.0f, cp);
		Assert.assertEquals("0.0f", 0.0f, fi.getFloatValue(), 0f);
		fi = new FloatInfo(-1.0f, cp);
		Assert.assertEquals("-1.0f", -1.0f, fi.getFloatValue(), 0f);
		fi = new FloatInfo(618.8f, cp);
		Assert.assertEquals("618.8f", 618.8f, fi.getFloatValue(), 0f);
		fi = new FloatInfo(Float.NEGATIVE_INFINITY, cp);
		Assert.assertEquals("negative infinity", Float.NEGATIVE_INFINITY, fi.getFloatValue(), 0f);
		fi = new FloatInfo(Float.POSITIVE_INFINITY, cp);
		Assert.assertEquals("positive infinity", Float.POSITIVE_INFINITY, fi.getFloatValue(), 0f);
		fi = new FloatInfo(Float.NaN, cp);
		Assert.assertEquals("not a number", Float.NaN, fi.getFloatValue(), 0f);
	}
